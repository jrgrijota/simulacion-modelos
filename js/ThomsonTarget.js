class ThomsonTarget {
  constructor(x, y, radius, numElectrons, isSimplified = false) {
    ...
    this.ke = 45.0;          // Constante electrostática global
    this.qAlpha = 2.0;       
    this.Z = numElectrons;   
  ...
  generateDiscretizedAtom() {
    ...
    // 1. DISTRIBUIR LOS N ELECTRONES EN ANILLOS CONCÉNTRICOS
    let remaining = n;
    ...
    // 2. MATEMÁTICA INTACTA: Distribución en Espiral de Fermat para la masa positiva de fondo
  ...
  calculateNetForce(alpha) {
    ...
    // REINYECCIÓN DE LA FUERZA DISPERSIVA EN MODO LÁMINA
    if (this.isSimplified) {
      if (distToCenter > this.R) return fNet; // Fuera del miniátomo sigue siendo neutro

      // Dentro del miniátomo: Fuerza repulsiva central acoplada para simular las colisiones de la lámina
    ...
    // MATEMÁTICA VISTA ATÓMICA VIGENTE (Suma discreta todos contra todos)
    let softeningSq = pow(14.0, 2); 

    // Sumatorio de repulsión de la masa positiva discreta
    for (let pPos of this.positivePoints) {
      ...
      let fPositive = rVec.copy().normalize().mult(magP);
      fNet.add(fPositive);
    }

    // Sumatorio de atracción de los electrones discretos
    for (let ePos of this.electrons) {
      ...
      let fElectron = rVec.copy().normalize().mult(magE);
      fNet.add(fElectron);
    }
  ...
  display() {
    if (!this.isSimplified) {
      // REDISEÑO GRÁFICO: Fondo naranja pálido uniforme continuo para la masa positiva
      fill(255, 110, 0, 28); 
      stroke(255, 130, 0, 60);
      strokeWeight(1);
      ellipse(this.pos.x, this.pos.y, this.R * 2, this.R * 2);

      // Centro geométrico sutil
      fill(255, 50, 50, 80);
      noStroke();
      ellipse(this.pos.x, this.pos.y, 2, 2);

      // Electrones dibujados estrictamente como puntos azules nítidos de 1px
      fill(0, 160, 255);
      noStroke();
      ...
      // Visualización para el modo lámina (Zoom bajo)
      fill(255, 190, 0, 20);
      stroke(255, 180, 0, 35);
      strokeWeight(0.6);
      ellipse(this.pos.x, this.pos.y, this.R * 2, this.R * 2);
